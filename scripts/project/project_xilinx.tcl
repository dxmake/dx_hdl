...
## Define the IGNORE_VERSION_CHECK environment variable to skip version check
...
## dx_project variables
set prj_mode 0
set project_system_dir ""
...
# system_top module)
#
proc dx_project {project_name {mode 0} {parameter_list {}} } {
  ...
  if {$IGNORE_VERSION_CHECK} {
    ...
    if {[string compare $VIVADO_VERSION $REQUIRED_VIVADO_VERSION] != 0} {
      ...
      puts -nonewline "expected $REQUIRED_VIVADO_VERSION, "
      puts -nonewline "got $VIVADO_VERSION.\n"
      puts -nonewline "This ERROR message can be down-graded to CRITICAL WARNING by setting IGNORE_VERSION_CHECK environment variable to 1. Be aware that ADI will not support you, if you are using a different tool version.\n"
      exit 2
    }
  }
  ...
  set lib_dirs $dx_hdl_dir/library

  # Set a common IP cache for all projects
  if {$USE_OOC_SYNTHESIS == 1} {
    if {[file exists $dx_hdl_dir/ipcache] == 0} {
      file mkdir $dx_hdl_dir/ipcache
    }
    config_ip_cache -import_from_project -use_cache_location $dx_hdl_dir/ipcache
  }
  ...
  source $dx_hdl_dir/scripts/project/msg_xilinx.tcl
...
#
proc dx_project_files {project_name project_files} {
...
# \param[project_name] - name of the project
#
proc dx_project_bd {project_name} {
...
# \param[project_name] - name of the project
#
proc dx_project_run {project_name} {
  ...
  if {![info exists ::env(NO_BITSTREAM_COMPRESSION)] && ![info exists NO_BITSTREAM_COMPRESSION]} {
  ...
  if {[info exists ::env(GENERATE_UTILIZATION)]} {
    ...
    puts "GENERATE_REPORTS: Resource utilization files won't be generated because GENERATE_UTILIZATION env var is not set"
  }

  if {[info exists ::env(GENERATE_XPA)]} {
    ...
    puts "GENERATE_REPORTS: Power analysis files won't be generated because GENERATE_XPA env var is not set"
  }
...
#
proc dx_project_synth {project_name prcfg_name hdl_files {xdc_files ""}} {
...
# \param[xdc_files] - XDC constraint source of the partial design
#
proc dx_project_impl {project_name prcfg_name {xdc_files ""}} {
...
# \param[project_name] - project name
#
proc dx_project_verify {project_name} {
